...
\end{defn}
\begin{eg}
    Sets: Let $X,Y,Z$ be sets then define $X\cup Y=(X \times \{0\}) \cup (Y \times \{1\})$. Define $i:x \mapsto (x,0)$ and $j:y \mapsto (y,1)$. Then the coproduct cone is $(X\cup Y, i, j)$. The unique $\phi: X\cup Y \to Z$ such that $\phi \circ i = f$ and $\phi \circ j = g$ is $\phi(x)=f(x)$ if $x\in X$ and $\phi(y)=g(y)$ if $y\in Y$.
\end{eg}
\end{document}
...
\end{lem}
\begin{defn}
    Let $\mathcal{C}$ and $\mathcal{D}$ be categories. A functor from $\mathcal{C}$ to $\mathcal{D}$ is $\mathcal{C} \xrightarrow{F} \mathcal{D}$. Assign to each object $a\in \mathcal{C}$ an object $F(a)$ of $\mathcal{D}$
\end{defn}
\end{document}
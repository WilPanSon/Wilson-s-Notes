...
\section{March 18}
\begin{thm}
    If $M$ finite generated $R$ module and $R$ PID then 
    \[
    M\simeq \frac{R}{(c_1)}\oplus \cdots \oplus \frac{R}{(c_n)} \oplus R^t
    .\] 
    for $c_i\neq 0,c_i | c_{i+1}$ and $c_i$ nonunit. 
\end{thm}
\begin{lem}
    For any $R$ in which $1\neq 0$, $R^n$ free $R$-module of rank $n$ with basis $e_i=(0,...,1,...,0)$ then $Hom(R^n,R)\simeq R^n$ generated by $e_j^*:(\lambda_1,...,\lambda_n)\to \lambda_j$
\end{lem}
\begin{lem}
    $R$ PID, $N$ free module of rank $n$ and $M\leq N$ then 
    \begin{enumerate}
        \item $M$ free module of rank $m, m\leq n$
        \item There is a basis $z_1,...,z_n$ of $N$ with nonzero $d_1,...,d_m$ such that $d_i | d_{i+1}$ and $d_1z_1,...,d_m z_m$ basis for $M$
    \end{enumerate}
\end{lem}
\begin{proof}
    The construction is as follows:\\
    I will find $z\in N, z\neq 0$ and $d\neq 0$ such that $dz\in M$, $Rz$ is direct summand of $N$ and $R(dz)$ is direct summand of $M$. \\
    We claim that there is $\psi\in Hom(N,R)$ such that $\psi[M]\neq 0$\\
    To show this claim WLOG $N=R^n$, let $m=(a_1,...,a_n)\in M$, $m\neq 0$ so $a_j\neq 0$ for some $j$, $e_j^*(m)=a_j\neq 0$ so $e_j^*[M]\neq 0$.\\
    Note: If $M\leq N$ and $\psi:Hom(N,R)$ then $\psi[M]$ is an ideal of the PID $R$.\\
    Consider the set of ideals $ \{\psi[M]:\psi\in Hom(N,R)\}.$. This is a nonempty set of ideals and contains a nonzero ideal. \\
    As $R$ is netherian this set has a maximal element. Choose $\psi, d$ such that $\psi[M]=(d)$ is maximal in $ \{\phi[M]:\phi\in Hom(N,R)\}$.\\
    Choose $y\in M, \psi(y)=d$ \\
    As $R$ PID, $(a,b)=(\gcd(a,b))=(\lambda a+\mu b)$. \\
    We claim: $d|\phi(y)$ for all $\phi\in Hom(N,R)$. \\
    To show this, let $d'=\phi(y)$ and consider $\gcd(d,d')=e$. 
    \[
    e=\lambda d + \mu d'=\lambda \psi(y)+\mu \phi(y)=(\underbrace{\lambda \psi + \mu \phi}_{\in Hom(N,R)})(y)\in (\lambda \psi + \mu \phi)[M]
    .\]
    So 
    \[
        e|d\implies \psi[M]=(d)\subseteq (e)\subseteq (\lambda \psi +\mu \phi)[M] 
    .\]  
    By maximal choice of $\psi[M]$, $(d)=(e)$ so $d,e$ associates so $d|d'$. \\
    Let $w_1,...,w_n$ be a basis of $N$. Let $w_i^*\in Hom(N,R)$ and $\mu_i^* \left( \sum_{i=1}^n \mu_j w_j \right)=\mu_i$. Let $y=\sum_{i=1}^n\mu_j w_j$. \\
    By our claim, $\mu_j|w_j^*(y)=\mu_j$ for all $j$. Let $z=\sum_{i=1}^n \left( \frac{\mu_j}{d} \right)w_j\in N$. \\
    Note that $dz=y$ as $\psi(y)=d$ as $d \psi(z), \psi(z)=1$. \\
    Our final claim is that 
    \begin{enumerate}
        \item $\ker(\psi)$ and $Rz$ are complementary in $N$
        \item $\ker(\psi)\cap M$ and $Ry$ complementary in $M$
    \end{enumerate}
    To show (a), easy to use $\ker(\psi)\cap Rz=0$ as $\psi(z)=1$. Let $n\in N, \lambda=\psi(n)$, $n=(n-\lambda z)+\lambda z$, $\psi(n-\lambda z)=\lambda-\lambda=0$ and (b) is similar. 
\end{proof}
\begin{proof}
    To show 1, of lemma 24.3, by induction on rank of $M$ with $N$ fixed, Suppose $1$ have it for submodules of $N$ of smaller rank. Using our basic construction, $M$ is decomposed into $Ry$ and $\ker(\psi)\cap M$. It's easy to see that the rank of $\ker(\psi)\cap M$ (add $y$ to an independent set in $\ker(\psi)\cap M$). \\
    By induction hypothesis, $\ker(\psi)\cap M$ is free and adding $y$ to some basis of $\ker(\psi)\cap M$ to get basis for $M$. \\
    TO prove 2, of lemma 24.3, by induction on rank $(n)$ of $N$ for all $M\leq N$ simultaneously.
\end{proof}
\end{document}
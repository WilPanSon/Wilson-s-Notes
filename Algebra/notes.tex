...
\begin{proof}
    ...
    (Induction Step) Let $h\in I$ with $\deg(h)=n>N$, $h=rx^n+\sum_{i<n}c_ix^i$ with $r\in J_n=J_N$. Find $g_1',...,g_s'\in F_N, \lambda_1,...,\lambda_s\in R$ such that $r=$ leading coefficients of $\sum_{j=1}^s \lambda_j g_j'$ with leading term in $rx^N$. So now look at $h-\sum_{j=1}^s\lambda_j x^{n-N}g_j'$  
\end{proof}
\end{document}
...
\begin{thm}
    ...
    \begin{proof}
        Since $ \{\overline{x}: x\in X\}$ generates $\overline{F}$ there is at most one $\tau$. By universal property of $F$, there is unique homomorphism $\phi:F\to S$ such that \[
        ...
        By choice of $N$, $N\leq \ker(\phi)$, we'll attempt to define $\tau:\overline{F}\to G$ by $\tau(wN)=\psi(w)$ for $w\in F$. \\

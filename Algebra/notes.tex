...
\begin{proof}
    ...
    Let $i\geq j$, we've already shown $K_j\subseteq K_i$ so we just have to show that $K_i \subseteq K_j$. Let $k\in K_i$ then $k+N\in \frac{K_i+N}{N}=\frac{K_j+N}{N}$. So there is $k'\in K_j$ such that $k+N=k'+N$ and so $k-k'\in N$ 
\end{proof}
\end{document}
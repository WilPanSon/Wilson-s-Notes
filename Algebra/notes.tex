...
\end{defn}
\begin{eg}
    Sets: Let $X,Y,Z$ be sets then define $X\cup Y=(X \times \{0\}) \cup (Y \times \{1\})$. The coproduct cone is $(X\cup Y, \iota_1, \iota_2)$ where $\iota_1(x)=(x,0)$ and $\iota_2(y)=(y,1)$. The unique $\rho:X\cup Y\to Z$ with $\rho(x)=f(x)$ and $\rho(y)=g(y)$ makes the diagram
\end{eg}
\end{document}
...
\end{lem}
\begin{proof}
    Let $(K_i)_i$ with $K_0\subseteq K_1\subseteq \cdots$ and $K_i$ submodules of $M$. Then $(K_i\cap N)_i$ stabilizes as $K_I\cap N$ are noetherian. Consider $(\phi_N[K_i])_i$ with $\phi_N:M\to M/N$ stablizes. Let $j$ be so large that $i\geq j$, $K_i\cap N=K_j \cap N$ and $\frac{K_i+N}{N}=\frac{K_j\cap N}{N}$.  
\end{proof}
\end{document}
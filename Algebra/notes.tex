...
\end{lem}
\begin{proof}
    Let $e_i= (0,...,1,...,0)$ where $1$ is in the $i$-th position. We have $e_1,...,e_n$ are independent and $| \{e_1,...,e_n\} |=n$. Suppose $I\subseteq R^n$ is independent and $|I|=n+1$. Then $I$ contains a non-zero element $a= (a_1,...,a_n)$ so $a_i\neq 0$ for some $i$. We have $a_i e_i=0$ so $a_i=0$ contradiction.
\end{proof}
\end{document}
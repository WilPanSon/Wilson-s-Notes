...
\section{Mar 23}
\begin{defn}
    Let $F:\mathcal{C}\to\mathcal{D}$ and $G:\mathcal{D}\to \mathcal{C}$ with $\mathcal{C,D}$ categories and $F,G$ functors. An adjunction between $F$ and $G$ is a family of bijections $v_{cd}:Hom_{\mathcal{D}}(Fc,d)\simeq Hom_{\mathcal{C}(c,}$ for objects $c$ of $\mathcal{C}, d$ of $\mathcal{D}$ such that 
\end{defn}
\end{document}
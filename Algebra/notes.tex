...
\end{lem}
\begin{defn}
    Let $\mathcal{C}$ and $\mathcal{D}$ be categories. A functor from $\mathcal{C}$ to $\mathcal{D}$ is $\mathcal{C} \xrightarrow{F} \mathcal{D}$. Assign to each object $a$ of  $\mathcal{C}$ an object $F(a)$ of $\mathcal{D}$. For each arrow $a \xrightarrow{f} b$ there is an error $F(a)\xrightarrow{F(f)}$
\end{defn}
\end{document}
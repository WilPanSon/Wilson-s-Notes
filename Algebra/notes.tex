...
\end{eg}
\begin{defn}
    Given $M,N$ $R$-modules we will construct an $R$-module $M\otimes_R N$ (tensor product of $M$ and $N$) together with a bilinear map $\otimes:M\times N\to M\otimes_R N$ by $(m,n)\mapsto m\otimes n$ such that for all $R$-module $P$ and all bilinear $\phi:M\times N\to P$ there is unique $R$-linear 
\end{defn}
\end{document}
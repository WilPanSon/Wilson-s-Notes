...
\section{March 18}
\begin{defn}
    The VC dimension of $F$ is $d_F=\max \{d | \exists A, |A|=d, |F_A|=2^{|A|}\}$
\end{defn}
\begin{thm}
    Saver-Shelah Theorem: 
    \[
    t_F(n)\leq \sum_{j=0}^{d_F} {n \choose j}
    .\] 
    This implies there exist two cases for any $X,F$, either $t_F(n)=2^n$ for all $n$ or $\exists d$ such that $t_F(n)=O(n^d)$
\end{thm}
\begin{proof}
    By induction on $n$, let $d_F<\infty$ be VC-dim of $F$. Now to show $\forall A, t_F(A)\leq \sum_{j=0}^{d_F}{|A| \choose j}$. WLOG, let $A=[n]$, $t_F(A)=|F_{[n]}|$ where $F_{[n]}=\{f\cap [n] | f\in F\}$. Define 
    \[
    F_0= \{f\in F | f\in F_{[n]}\} \text{ and } F_1= \{f\in F_{[n-1]}|f\cup \{n\}\in F_{[n]}\}
    .\] 
    We have \[
    |F_{[n]}|=|F_0|+|F_1|=|F_0\cup F_1|+|F_0\cap F_1|
    .\] 
    and $F_0\cup F_1=F_{[n-1]}$. By induction we have 
    \[
    |F_{[n]}|\leq \sum_{j=0}^d {n-1 \choose j}+|F_0\cap F_1|
    .\] 
    We have $d_{F_0\cap F_1}\leq d_{F_{[n]}}-1\leq d_F-1$. \\
    Since for any $A\subseteq [n-1]$ shattered by $F_0\cap F_1$, $A\cup \{n\}$ is shattered by $F_{[n]}$. So 
    \[
    |F_0\cap F_1|\leq \sum_{j=0}^{d} {n-1 \choose j}+\sum_{j=-1}^{d-1}{n-1 \choose j}=\sum_{j=0}^{d}{n\choose j}
    .\] 
\end{proof}
\end{document}
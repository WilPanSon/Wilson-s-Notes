...
\end{prop}
\begin{proof}
    Consider a matching of size $\mu$ in $H$ which has $m$ edges. I get a transversal of size $\tau=k\mu$. If $\mu\geq r$, we're done. Otherwise, I get a transversal of size $\tau=k\mu$. There is a vertex $v$ such that $v$ is in $\geq \frac{m}{\tau}\geq \frac{m}{k(r-1)}$ 
\end{proof}
\end{document}
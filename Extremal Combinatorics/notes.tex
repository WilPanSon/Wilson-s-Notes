...
\section{March 16}
\begin{thm}
    Let $X=[0,1]^2$, $F$ is family of axis parralel rectangles. Write $S_n= \{X_1,...,X_n\}$ be randomly i.i.d points sampled from $X$. Define 
    \[
        \mu(f)=\Pr[x_i\in f] \text{ and } \mu_{ \{x_i\}^n}(f)= \frac{1}{n}\sum \mathbb{I}_{x_i\in f}
    .\] 
    Can we bound 
    \[
    \Pr[ \sup_{f\in F} |\mu_{ \{x_i\}^n}(f)-\mu(f)|>\epsilon]
    .\] 
\end{thm}
\begin{rmk}
    If I fix a point set $S_n$, then there are $\leq n^4$ different rectangles. However this would depend on the point set. 
\end{rmk}
\begin{lem}
    \[
    \Pr[\underbrace{\sup_f \mu(f)-\mu_{ \{x_i\}^n}(f)>\epsilon}_{ \text{event $E$}}]\leq 2 \Pr[\sup_f \mu_{ \{y_i\}^n} (f)-\mu_{ \{x_i\}^n}(f)> \epsilon/2]
    .\] 
    Where $ \{y_i\}$ is an independent sequence of i.i.d samples. \\
\end{lem}
\begin{proof}
    When event $E$ happens, choose some $f_E$ witnessing the event $\mu(f_E)-\mu_{ \{x_i\}^n}(f_E)>\epsilon$. Then by Chebyshev's inequality
    \[
    \Pr\left [\mu_{ \{y_i\}^n}(f_E)-\mu(F_E)> \frac{\epsilon}{2}\right ] \leq \frac{1/4n}{\epsilon^2/4}= \frac{1}{\epsilon^2 n}\leq \frac{1}{2}
    .\] 
    Fix now any $\overline{X_1},...,\overline{X_n}\in E$. For this choice $\mu(f_E)-\mu_{ \{x_i\}^n}(f_E)>\epsilon$. In particular for these fixed events \[
    \Pr[\mu_{ \{y_i\}^n}(f_E)-\mu_{ \{ \overline{X_i} \}^n}(f_E)>\epsilon/2]\leq \frac{1}{2}
    .\] 
    Averaging over choices for $\overline{X_1},...,\overline{X_n}\in E$ we get 
    \[
        \Pr[\mu_{ \{y_i\}^n}(f_E)-\mu_{ \{ \overline{X_i} \}^n}(f_E)>\epsilon/2]\leq \frac{1}{2}\Pr[E]
    .\] 
\end{proof}
\begin{proof}
    To prove the main result, we want to bound 
    \[
        \Pr( \sup_{f\in F} |\mu_{ \{x_i\}^n}(f)-\mu(f)|>\epsilon)
    .\] 
    We'll start with the one sided version 
    \[
        \Pr( \sup_{f\in F} \mu(f)-\mu_{ \{x_i\}^n}(f)>\epsilon)\leq 2 \Pr[\sup_f \mu_{ \{y_i\}^n} (f)-\mu_{ \{x_i\}^n}(f)> \epsilon/2]
    .\] 
    Let $\Omega$ be the choices of the $x_i$'s and $y_i$'s. I can generate a sample in $\Omega$ by 
    \begin{enumerate}
        \item Choose $x_1,...,x_n,y_1,...,y_n$ i.i.d uniform in $[0,1]^2$ or 
        \item Choose $x_1,...,x_n,y_1,...,y_n$ i.i.d uniform in $[0,1]^2$ then flip $n$ random fair coins if coin $i$ is head exchange $x_i,y_i$. 
    \end{enumerate}
    We'll use 2. to generate $\Omega$. We'll only use randomness of the coin flips. We'll prove upper bound on $**$ that works for any choice of $ \{x_1,y_1\}, \{x_2,y_2\},...$ as long as the coin flips are random. \\
    We are essentially letting the adversary choose the points and using randomness of the coin flips to color the points. \\
    With respect to the fixed placement of the paired points, there are only $(2n)^4$ rectangles that matter. \\
    What really matters is that given $(X,F)$, let $t_F(A)=\# \{f\cap A | f\in F\}$ where $A$ is the set of our sample points $x_1,...,x_n,y_1,...,y_n$. \\
    In our argument, for $F=$ axis parallel rectangles, $t_F(A)\leq |A|^4$. We just need this to be polynomial. \\
    So letting $t_F(n)=\max_{|A|=n} t_F(A)$ we have 
    \[
    \Pr[\sup_{f\in F} (\mu(f)-\mu_{ \{x_i\}^n}(f))>\epsilon]\leq 2 t_F(2n)\cdot 2^{-2n\epsilon^2}
    .\] 
    To get the other direction, 
    \[
        \Pr[\sup_{f\in F} (\mu_{ \{x_i\}^n}(f)-\mu(f))>\epsilon]\leq 2 t_F(2n)\cdot 2^{-2n\epsilon^2}
    .\] 
    by $\overline{F}= \{X\setminus f | f\in F\}$
\end{proof}
\newpage
\section{March 18}
\begin{defn}

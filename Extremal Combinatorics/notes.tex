...
\end{thm}
\begin{proof}
    Given $F$-free graph $G$, apply Regularity Lemma and clean it up to obtain $G'$ whose reduced graph is $R$ where $R=R(G)$, $G'=G'(G)$ and $k=|V(R)|$ also depends on $G$ and satisfies $k\leq M(\epsilon)$  
\end{proof}
\end{document}
...
\begin{proof}
$V_1, \dots, V_k$ are the main parts, all of equal size. \\
\textbf{The Assumption:} The partition $\mathcal{P}$ is \textbf{not} $\epsilon$-regular. \\

By definition, if a partition is not $\epsilon$-regular, it means the total volume of all $\epsilon$-irregular pairs $(V_i, V_j)$ exceeds an $\epsilon$-fraction of the total possible volume. Mathematically, this means:
\[
    \sum_{(V_i, V_j) \text{ is irregular}} \frac{|V_i||V_j|}{|V|^2} > \epsilon
\]
\textbf{Our goal:} Construct a refined partition $\mathcal{P}_{\text{new}}$ and prove that $f(\mathcal{P}_{\text{new}}) \ge f(\mathcal{P}) + \epsilon^5$. \\

\vspace{0.5cm}

\textbf{Step 1: Isolate the "Witness" Sets} \\
Because every $\epsilon$-irregular pair $(V_i, V_j)$ is irregular, there must exist sub-parts that prove it. Let's call them $A^{(i,j)} \subset V_i$ and $B^{(i,j)} \subset V_j$. By the definition of irregularity, they satisfy: \\
1. \textbf{Size:} $|A^{(i,j)}| \ge \epsilon|V_i|$ and $|B^{(i,j)}| \ge \epsilon|V_j|$ \\
2. \textbf{Density Deviation:} $|d(A^{(i,j)}, B^{(i,j)}) - d(V_i, V_j)| \ge \epsilon$ \\

For every irregular pair involving $V_i$, this gives us a simple boolean cut of $V_i$: $\{A^{(i,j)}, V_i \setminus A^{(i,j)}\}$. \\

\vspace{0.5cm}

\textbf{Step 2: The Common Refinement} \\
A single part $V_i$ might be involved in multiple irregular pairs. To capture all the density deviations, we take the \textbf{common refinement} of all these boolean cuts on $V_i$. \\

Let $\mathcal{P}_i$ be this new, finely chopped partition of $V_i$. Because $V_i$ interacts with at most $k$ other sets, we cut it at most $k$ times. Therefore, $\mathcal{P}_i$ contains at most $2^k$ sub-parts. \\

Our completely new partition of the whole graph is $\mathcal{P}_{\text{new}} = \bigcup_{i=1}^k \mathcal{P}_i \cup \{V_0\}$. \\

\vspace{0.5cm}

\textbf{Step 3: Energy Increment for ONE Irregular Pair} \\
Now we apply the Lemma. For any specific $\epsilon$-irregular pair $(V_i, V_j)$, we replace them with their refinements $\mathcal{P}_i$ and $\mathcal{P}_j$. \\

According to the Lemma, the new energy between these refined parts is at least the old energy \textit{plus} the variance caused by the witness sets $A$ and $B$:
\begin{align*}
    f(\mathcal{P}_i, \mathcal{P}_j) &\ge f(V_i, V_j) + \left( d(A,B) - d(V_i, V_j) \right)^2 \frac{|A||B|}{|V|^2} \\
    &\ge f(V_i, V_j) + (\epsilon)^2 \frac{(\epsilon|V_i|)(\epsilon|V_j|)}{|V|^2} \\
    &= f(V_i, V_j) + \epsilon^4 \frac{|V_i||V_j|}{|V|^2}
\end{align*}
\textbf{For regular pairs}, we don't have a guaranteed variance boost, so we just use the trivial bound: $f(\mathcal{P}_i, \mathcal{P}_j) \ge f(V_i, V_j)$. \\

\vspace{0.5cm}

\textbf{Step 4: The Global Energy Increment} \\
Now we sum the energy over all pairs in our new partition to find the total energy $f(\mathcal{P}_{\text{new}})$. We split the sum into pairs that were regular and pairs that were irregular (dropping the Junk set $V_0$ entirely to form a lower bound). \\

\begin{align*}
    f(\mathcal{P}_{\text{new}}) &\ge \sum_{(i,j) \text{ regular}} f(\mathcal{P}_i, \mathcal{P}_j) + \sum_{(i,j) \text{ irregular}} f(\mathcal{P}_i, \mathcal{P}_j) \\
    &\ge \sum_{\text{regular}} f(V_i, V_j) + \sum_{\text{irregular}} \left[ f(V_i, V_j) + \epsilon^4 \frac{|V_i||V_j|}{|V|^2} \right] \\
    &= f(\mathcal{P}) + \epsilon^4 \sum_{(V_i, V_j) \text{ irregular}} \frac{|V_i||V_j|}{|V|^2}
\end{align*}

We use the definition of an $\epsilon$-irregular partition from the Setup section. We know that the sum of the volume of irregular pairs is strictly greater than $\epsilon$. Substitute that into the equation:
\begin{align*}
    f(\mathcal{P}_{\text{new}}) &\ge f(\mathcal{P}) + \epsilon^4 (\epsilon) \\
    &= f(\mathcal{P}) + \epsilon^5
\end{align*}
\end{proof}

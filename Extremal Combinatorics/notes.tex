...
\begin{lem}[\textbf{Triangle Counting Lemma}]
    Let $X,Y,Z\subset V(G)$ be disjoint with $d(X,Y), d(X,Z), d(Y,Z)\geq 2\epsilon$, and suppose $(X,Y)$, $(X,Z)$, $(Y,Z)$ are $\epsilon$-regular. Then the number of $XYZ$-triangles is at least
    \begin{align*}
        &(1-2\epsilon)(d(X,Y)-\epsilon)(d(X,Z)-\epsilon)\\&\quad\times(d(Y,Z)-\epsilon).
    \end{align*}
\end{lem}
\end{document}
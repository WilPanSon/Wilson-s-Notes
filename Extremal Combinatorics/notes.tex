...
\begin{proof}
    ...
    .\] 
    Put an edge between lines $\ell, \ell'$ if $\ell \cap \ell'\in A$. Each point of $A$ implies there are $3$ edges of $G$ that form a $\triangle$ in $G$. Furthermore, these $\triangle$'s are edge disjoint. Edge disjointness implies to kill all $\triangle$'s in $G$ we need to delete at least $|A|=\beta n^2$ edges. By triangle removal the number of $\triangle$'s in $G$ is at least $\delta(\beta) n^3$. 
\end{proof}
\end{document}
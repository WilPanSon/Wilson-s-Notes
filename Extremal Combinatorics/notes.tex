...
\end{thm}
\begin{proof}
    Assume WLOG $0\in CH(x_1,...,x_m)$ but not in $CH(x_{i_1},...,x_{i_{d+1}})$. For any $| \{i_1,...,i_{d+1}\}=d+1$. For any $x_i,$ define $H_i= \{v| v\cdot x_i\geq 1\}$. For any choice of $x_{i_1},...,x_{i_{d+1}}$ we have $0\in CH(H_{i_1},...,H_{i_{d+1}})$. By Helly's Theorem, $\exists v$ such that $v\cdot x_i\geq 1$ for all $i\in [d+1]$. So $v\cdot x_i\geq 1$ for all $i\in [m]$. So $0\in CH(x_1,...,x_m)$. Contradiction. 
\end{proof}
\end{document}
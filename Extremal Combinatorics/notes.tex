...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item Let $\mathcal{P}$ be the old partition and $\mathcal{P}_{ \text{new}}$ be the new partition. 
        \[
        f(\mathcal{P}_{ \text{new}})\leq f(\mathcal{P})+\sum_{i,j} \left( d(V_i, V_j)-d(V_{i,1}^{(i,j)}, V_{j,1}^{(i,j)}) \right)^2\frac{|V_{i,1}^{(i,j)}||V_{j,1}^{(i,j)}|}{|V|^2}+\sum_{i,j} \left( d(V_i, V_j)-d(V_{i,2}^{(i,j)}, V_{j,2}^{(i,j)}) \right)^2\frac{|V_{i,2}^{(i,j)}||V_{j,2}^{(i,j)}|}{|V|^2}
        .\] 
    \end{enumerate} 
\end{proof}
\end{document}
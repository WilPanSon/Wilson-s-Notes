...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item For each $\epsilon$-irregular pair $(V_i,V_j)$ there exist partitions $V_i=V_{i,1}^{(i,j)}\cup V_{i,2}^{(i,j)}$ and $V_j=V_{j,1}^{(i,j)}\cup V_{j,2}^{(i,j)}$ such that $|d(V_i, V_j)-d(V_{i,1}^{(i,j)}, V_{j,1}^{(i,j)})|\geq \epsilon$ and $|d(V_i, V_j)-d(V_{i,2}^{(i,j)}, V_{j,2}^{(i,j)})|\leq \epsilon$
        \\

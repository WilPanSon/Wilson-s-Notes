...
\begin{thm}
    ...
    |\mathcal{F}|\leq \begin{cases}
        ...
        {n-1 \choose k-1} & \text{if $k>\frac{n}{2}$}
    \end{cases}
    .\] 
\end{thm}
\begin{proof}
    Pick a random permutation $\pi\in S_n$ and put elements $\pi(1),...,\pi(k)$ on a circle. Let $|mathcal
    A$ be all $k$-element sets made of consecutive elements on the circle. \\
    Let $\mathcal{A}$ be all $k$-element sets made of consecutive elements on the circle. 
    \[
    \mathcal{A}:= \{ \{\pi(i),\pi(i+1),...,\pi(i+k-1) \}: i\in [n]\}
    .\] 
    \[
    \E[|\mathcal{A}\cap F|]=\sum_{A\subset \mathcal{F}} \Pr[A\in \mathcal{A}]=\frac{|F|}{{n \choose k}}n
    .\] 
\end{proof}
\end{document}
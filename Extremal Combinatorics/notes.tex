...
\end{thm}
\begin{proof}
    Partition the points into two sets $A$ and $B$ of size $n$ and $n+1$ respectively. $A$ is $STS(n)$ and $B$ has $n$ perfect matchings. For a vertex in $A$ assign it matching in $B$. If a pair is $A$ then take the triple in $STS(n)$. If they are both in $B$ then 
\end{proof}
\end{document}
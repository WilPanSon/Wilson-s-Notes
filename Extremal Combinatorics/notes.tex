...
\begin{proof}
    For a partition $\mathcal{P}$ of $w$ into sub-words $\mathcal{P}= \{w^{(1)},...,w^{(m)}\}$ we define $f(\mathcal{P})=\sum_{w'\in \mathcal{P}}d(w')^2|w'|/|w|$.\\
    Alternatively, we pick a symbol of $w$ at random, say $i\in [n]$ uniformly, then pick another symbol at random from the same part as the $1$'st, say $j$ is picked. By expanding the square, we can explicitly show how this relates to $f(\mathcal{P})$:
    \begin{align*}
        \Pr[w_i=w_j] &= \sum_{w'\in \mathcal{P}}\frac{|w'|}{|w|}(d(w')^2+(1-d(w'))^2) \\
        &= \sum_{w'\in \mathcal{P}}\frac{|w'|}{|w|}(2d(w')^2 - 2d(w') + 1) \\
        &= 2\sum_{w'\in \mathcal{P}}d(w')^2\frac{|w'|}{|w|} + \sum_{w'\in \mathcal{P}}\frac{|w'|}{|w|} - 2\sum_{w'\in \mathcal{P}}d(w')\frac{|w'|}{|w|} \\
        &= 2f(\mathcal{P}) + 1 - 2d(w)
    \end{align*}

    \begin{lem}
        If $\mathcal{P}'$ is a refinement of $\mathcal{P}$ (meaning each word in $\mathcal{P}'$ has a unique parent word in $\mathcal{P}$), then:
        \[
            f(\mathcal{P}')=f(\mathcal{P})+\sum_{w'\in \mathcal{P}'} \left( d(w')-d(\mathrm{parent}(w')) \right)^2 \frac{|w'|}{|w|}
        \]
    \end{lem}
    ...
    \begin{proof}[Proof of Lemma]
        Expanding the square in the second term we have:
        \[
            \sum_{w'\in \mathcal{P'}}d(w')^2\frac{|w'|}{|w|}-2d(w')d(\mathrm{parent}(w'))\frac{|w'|}{|w|}+d(\mathrm{parent}(w'))^2\frac{|w'|}{|w|}=\sum_1+\sum_2+\sum_3
        \]
        By definition, we immediately have $\sum_1 = f(\mathcal{P}')$. \\
        To simplify $\sum_2$ and $\sum_3$, we group the sub-words $w' \in \mathcal{P}'$ by their parent word $\tilde{w} \in \mathcal{P}$. We use two conservation facts: the lengths of the sub-words sum to the length of the parent ($\sum |w'| = |\tilde{w}|$), and the number of 1s is conserved ($\sum d(w')|w'| = d(\tilde{w})|\tilde{w}|$).

        For the second sum:
        \begin{align*}
            \sum_2 &= \sum_{\tilde{w}\in \mathcal{P}}\sum_{\substack{w'\in \mathcal{P'} \\ \mathrm{parent}(w')=\tilde{w}}} \left( -2d(w')d(\tilde{w})\frac{|w'|}{|w|} \right)\\
            &= \sum_{\tilde{w}\in \mathcal{P}}-2d(\tilde{w})\frac{1}{|w|}\sum_{\substack{w'\in \mathcal{P'} \\ \mathrm{parent}(w')=\tilde{w}}}d(w')|w'|\\
            &= \sum_{\tilde{w}\in \mathcal{P}}-2d(\tilde{w})\frac{1}{|w|} \cdot \left( d(\tilde{w})|\tilde{w}| \right)\\
            &= \sum_{\tilde{w}\in \mathcal{P}}-2d(\tilde{w})^2\frac{|\tilde{w}|}{|w|}\\
            &= -2f(\mathcal{P})
        \end{align*}
        ...
        \begin{align*}
            \sum_3 &=\sum_{\tilde{w}\in \mathcal{P}}\sum_{\substack{w'\in \mathcal{P'} \\ \mathrm{parent}(w')=\tilde{w}}} d(\tilde{w})^2 \frac{|w'|}{|w|}\\
            &=\sum_{\tilde{w}\in \mathcal{P}}d(\tilde{w})^2 \frac{1}{|w|} \sum_{\substack{w'\in \mathcal{P'} \\ \mathrm{parent}(w')=\tilde{w}}} |w'| \\
            &=\sum_{\tilde{w}\in \mathcal{P}}d(\tilde{w})^2 \frac{|\tilde{w}|}{|w|}\\
            &=f(\mathcal{P})
        \end{align*}
        So substituting these back, we have $\sum_1+\sum_2+\sum_3=f(\mathcal{P}') - 2f(\mathcal{P}) + f(\mathcal{P}) = f(\mathcal{P}')-f(\mathcal{P})$. This concludes the lemma.
    \end{proof}

    \noindent \textbf{Algorithm Analysis.}\\
    Start with the trivial partition (i.e. $\mathcal{P}= \{w\}$) and repeat the refinement process while the partition is not $\epsilon$-regular, which means:
    \[
        \sum_{\substack{w'\in \mathcal{P} \\ w' \text{ is } \epsilon\text{-irregular}}} |w'| > \epsilon |w|
    \]

    For each $w'\in \mathcal{P}$ that is $\epsilon$-irregular, find a way to write it as $w'=w^{(1)}w^{(2)}w^{(3)}$ where the middle chunk acts as the witness to irregularity, satisfying $|d(w^{(2)})-d(w')|\geq \epsilon$ and $|w^{(2)}|\geq \epsilon |w'|$.\\
    Replace each such irregular $w'$ with its three sub-words $w^{(1)},w^{(2)},w^{(3)}$ (keeping regular words unchanged) to obtain a new partition of $w$, call it $\mathcal{P}'$. Repeat with $\mathcal{P}$ replaced by $\mathcal{P}'$.\\

    To rigorously analyze the energy increase of this algorithm step, we apply the Lemma and strategically drop non-negative terms to establish a strict lower bound:
    \begin{align*}
        f(\mathcal{P}') &= f(\mathcal{P}) + \sum_{w'\in \mathcal{P}} \sum_{\substack{w''\in \mathcal{P}' \\ \mathrm{parent}(w'')=w'}} \left( d(w'')-d(w') \right)^2 \frac{|w''|}{|w|} \\
        &\geq f(\mathcal{P}) + \sum_{\substack{w'\in \mathcal{P} \\ w' \text{ is } \epsilon\text{-irregular}}} \sum_{\substack{w''\in \mathcal{P}' \\ \mathrm{parent}(w'')=w'}} \left( d(w'')-d(w') \right)^2 \frac{|w''|}{|w|} \\
        &\geq f(\mathcal{P}) + \sum_{\substack{w'\in \mathcal{P} \\ w' \text{ is } \epsilon\text{-irregular}}} \left( d(w^{(2)})-d(w') \right)^2 \frac{|w^{(2)}|}{|w|} \\
        &\geq f(\mathcal{P}) + \sum_{\substack{w'\in \mathcal{P} \\ w' \text{ is } \epsilon\text{-irregular}}}\epsilon^2 \frac{\epsilon |w'|}{|w|} \\
        &= f(\mathcal{P}) + \frac{\epsilon^3}{|w|} \sum_{\substack{w'\in \mathcal{P} \\ w' \text{ is } \epsilon\text{-irregular}}} |w'| \\
        &> f(\mathcal{P}) + \frac{\epsilon^3}{|w|} (\epsilon |w|) \\
        &= f(\mathcal{P})+\epsilon^4
    \end{align*} 

    Note: $f(\mathcal{P})\leq 1$ for any partition $\mathcal{P}$ because densities are bounded between $0$ and $1$.\\
    Because the energy strictly increases by at least $\epsilon^4$ in every step, the total number of steps in the algorithm is bounded by $\lfloor 1/\epsilon^4 \rfloor$. Since each step cuts an existing part into at most 3 parts, the total number of parts multiplies by at most 3 each time. Therefore, the algorithm must terminate, and the theorem holds with the maximum number of parts being $M(\epsilon)=3^{1/\epsilon^4}$.
\end{proof}

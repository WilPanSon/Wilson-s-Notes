...
\end{defn}
\begin{defn}
    A pair $(U,V)$ of disjoint subsets of $V(G)$ is $\epsilon$-regular if $\forall U'\subseteq U$ and $V'\subseteq V$ such that $|U'|\geq \epsilon |U|$ and $|V'|\geq \epsilon |V|$ we have \[
\end{defn}
\begin{thm}
    (Szemeredi's Regularity Lemma) For any $\epsilon>0$ and $n$-vertex graph $G$ there exists a partition $\mathcal{P}$ of $V(G)$ into $\leq n^{\epsilon}$ parts such that the pair $(A,B)\in \mathcal{P}\times \mathcal{P}$ is $\epsilon$-regular for all $A,B\in \mathcal{P}$.
\end{thm}
\end{document}
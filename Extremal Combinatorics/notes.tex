...
\section{Feb 16}
\begin{lem}
    Triangle Counting Lemma: Let $X,Y,Z\subset V(G)$ be disjoint such that $d(X,Y), d(X,Z),d(Y,Z)\geq 2\epsilon$ and pairs $(X,Y),(X,Z),(Y,Z)$ are $\epsilon$-regular. Then there are at least $(1-2\epsilon)(d(X,Y)-\epsilon)(d(X,Z)-\epsilon)(d(Y,Z)-\epsilon)$ $XYZ$ triangles 
\end{lem}
\end{document}
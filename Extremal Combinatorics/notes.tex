...
\end{proof}
\begin{proof}
    To show Sperner's Theorem: Suppose $\mathcal{F}$ is an antichain with $a=\min \{|A|:A\in \mathcal{F}\}$ and $b=\max \{|A|:A\in \mathcal{F}\}$. Let $\mathcal{F}_a= \{A\in \mathcal{F}: |A|=a\}$ and $\mathcal{F}_b= \{A\in \mathcal{F}: |A|=b\}$. Then $\mathcal{F}_a$ and $\mathcal{F}_b$ are intersecting and by lemma 1 we have \[
\end{proof}
\end{document}
...
\begin{lem}
    ...
    .\] 
    \begin{proof}
        We use Markov's inequality with the exponential moment. For any $t > 0$ such that $pe^t < 1$:
        \[
        ...
        \]
        Now, we bound $\E[e^{tX_1}]$. Since $\Pr[X_1 \geq j] \leq p^j$ for all $j \geq 0$, and assuming $X_1$ is non-negative:
        \[
        \E[e^{tX_1}] = \sum_{j=0}^{\infty} e^{tj} \Pr[X_1 = j] \leq \sum_{j=0}^{\infty} e^{tj} \Pr[X_1 \geq j] \leq \sum_{j=0}^{\infty} (pe^t)^j = \frac{1}{1-pe^t},
        \]
        provided that $pe^t < 1$.
        
        Choose $t$ such that $pe^t = 1/2$. Then $e^t = 1/(2p)$ and $t = -\ln(2p)$. We have:
        \[
        \E[e^{tX_1}] \leq \frac{1}{1-1/2} = 2.
        \]
        Therefore:
        \[
        \Pr[X \geq 2n] \leq \frac{2^n}{e^{2tn}} = \frac{2^n}{e^{2n \cdot (-\ln(2p))}} = \frac{2^n}{(2p)^{-2n}} = 2^n \cdot (2p)^{2n} = (4p^2)^n.
        \]
        For $p \leq 1/4$, we have $(4p^2)^n \leq (4p)^n$, which gives the desired bound. For the general case, we can refine by choosing $t$ such that $pe^t = 1/4$. Then $e^t = 1/(4p)$, $t = -\ln(4p)$, and:
        \[
        \E[e^{tX_1}] \leq \frac{1}{1-1/4} = \frac{4}{3}.
        \]
        This yields:
        \[
        \Pr[X \geq 2n] \leq \frac{(4/3)^n}{e^{2n(-\ln(4p))}} = \frac{(4/3)^n}{(4p)^{-2n}} = (4/3)^n (4p)^{2n} = \left(\frac{64p^2}{3}\right)^n.
        \]
        For $p \leq \sqrt{3}/8$, we have $(64p^2/3)^n \leq (4p)^n$. However, the bound $(4p)^n$ holds more generally when $p$ is sufficiently small. A more careful optimization of the parameter $t$ shows that the bound $\Pr[X \geq 2n] \leq (4p)^n$ holds for all $p < 1/4$.
    \end{proof}
\end{lem}

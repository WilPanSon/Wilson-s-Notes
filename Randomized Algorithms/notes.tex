...
\section{Feb 23}
\begin{defn}
    Given $(X_1,d_1), (X_2,d_2),$ a map $\phi:X_1\to X_2$ has distortion $\alpha$ if there exists scaling parameter $\beta \neq 0$ such that \[
    \beta d_1(x,y)\leq d_2(\phi(x),\phi(y))\leq \alpha \beta d_1(x,y)
    .\] 
\end{defn}
\begin{thm}
    Bourgain's Theorem: Let $(x,d)$ be any $n$-pt metric space. Then there exist $\phi:(X,d)\to (\R^{O(\log^2 n)}, d_{\ell_1})$ such that distortion is $O(\log n)$
\end{thm}
\begin{defn}
    For $i=1,...,\log n$ and $j=1,...,c\log n$, define $S_{i,j}$ by including each $x\in X$ independently with probability $\frac{1}{2^i}$\\
    We define, \[
    \phi(x)=\gen{d(x,S_{1,1}),d(x,S_{1,2}),...,d(x,S_{\log n, c\log n})}
    .\] 
    where $d(x,S)=\min_{s\in S}d(x,s)$. 
\end{defn}
\begin{thm}
    With high probability in $n, \forall x,y\in X$ 
    \[
    \log n d(x,y)\leq d_{\ell_1}(\phi(x),\phi(y))\leq c\log^2 n d(x,y)
    .\]  
    With $\beta=\log n$ and $\alpha=c \log n$
\end{thm}
\begin{rmk}
    We can rewrite what we want to show as \[
        d_{\ell_1}(\phi(x),\phi(y)) = \sum_{i=1}^{\log n}\sum_{j=1}^{c\log n} \left |d(x,S_{i,j})-d(y,S_{i,j}) \right |\geq \log n d(x,y)
    .\] 
    with high probability
\end{rmk}
\begin{proof}
    Fix $x,y\in X$, define $B(x,r)= \{z\in X | d(x,z)<r\}$, define $r_i=\min r$ such that both $|B(x,r)|\geq 2^i$ and $|B(y,r)|\geq 2^i$. \\
    Define $t=\max \# t$ such that $r_t\leq \frac{d(x,y)}{2}$ and redefine $r_{t+1}:=\frac{d(x,y)}{3}$. 
    \begin{lem}
        For any $i=1,...,t+1$ and $j=1,...,c\log n$ with probability $\geq \frac{1}{10}$
        \[
            |d(x,S_{i,j})-d(y,S_{i,j})|\geq r_j-r_i \tag{*}
        .\] 
    \end{lem}
    \begin{claim}
        For any $i\leq t+1$, $\# j$ such that $(*)$ holds is $\geq \log n$ with high probability. \\
        This is shown by \# such $j=\sum$ independent 0-1 RV with mean $\geq \frac{c \log n}{10}$ implies by Chernoff bound that with high probability, \# such $j\geq \log n$ with high probability
    \end{claim}
    \noindent So we have 
    \[
        \sum_{i=1}^{\log n}\sum_{j=1}^{c\log n} \left |d(x,S_{i,j})-d(y,S_{i,j}) \right |\geq \sum_{i=1}^{t+1} \log n (r_i-r_{i-1})=\log (r_{t+1}-r_0)=\log(n) \frac{d(x,y)}{3}
    .\] 
    Observation \#1:  that either 
    \[
    |B(x,r_i)|< 2^i \text{ or } |B(y,r_i)|< 2^i
    .\] 
    Observation \#2: 
    \[
    |B(y,r_{i-1})|\geq 2^{i-1}
    .\] 
    If we miss one of the balls then $d(x,S_{i,j})-d(y,S_{i,j})\geq r_i-r_{i-1}$. So \[
    \Pr[S_{i,j} \text{misses }B(x,r_i)]\geq \left( 1-\frac{1}{2^i} \right)^{2i}\approx \frac{1}{e} \implies \Pr[S_{i,j} \text{ hits }B(x,r_i)]\leq 1- \left( 1-\frac{1}{2^i} \right)^{2i-1}\approx 1-\frac{1}{\sqrt{e}}\approx \frac{1}{3}
    .\] 
    So We miss with one of the balls with probability $\geq \frac{1}{e} \frac{1}{e}=\frac{1}{10}$
\end{proof}
\newpage
\section{March 16}
\begin{thm}

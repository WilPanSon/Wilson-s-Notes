...
\section{March 16}
\begin{thm}
    There exist hist independent algorithm with expected cost $\tilde{O}\left (\log^{1.5} n \right )$. 
\end{thm}
\begin{thm}
    There exist hist independent algorithm such that if instead is to a uniform random rank, then expected ins cost is $\tilde{O}(\log^{1.5} n)$. 
\end{thm}
\end{document}
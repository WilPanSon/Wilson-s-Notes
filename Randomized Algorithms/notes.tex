...
\begin{lem}
    ...
    \begin{proof}
        ...
        Choose $t$ such that $pe^t = 1/4$. Then $e^t = 1/(4p)$ and $t = -\ln(4p)$. We have:
        \[
        \E[e^{tX_1}] \leq \frac{1}{1-1/4} = \frac{4}{3}.
        \]
        Therefore:
        \[
        \Pr[X \geq 2n] \leq \frac{(4/3)^n}{e^{2tn}} = \frac{(4/3)^n}{e^{2n \cdot (-\ln(4p))}} = \frac{(4/3)^n}{(4p)^{-2n}} = (4/3)^n \cdot (4p)^{2n} = \left(\frac{64p^2}{3}\right)^n.
        \]
        For $p \leq \sqrt{3}/8$, this gives $(64p^2/3)^n \leq (4p)^n$. To achieve the bound $(4p)^n$ more generally, we optimize the choice of $t$. Setting the derivative of $\ln((1/(1-pe^t))/e^{2t})$ with respect to $t$ to zero, we find the optimal $t$ satisfies $pe^t = 1/4$, which we already used. For small $p$, the bound $(4p)^n$ follows from the fact that $(64p^2/3)^n \leq (4p)^n$ when $p \leq 3/16 \approx 0.1875$. More generally, by choosing $t$ appropriately, one can show that $\Pr[X \geq 2n] \leq (4p)^n$ holds for all $p$ sufficiently small.
    \end{proof}
\end{lem}

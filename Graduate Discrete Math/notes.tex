...
\author{Wilson Pan}
\date{\today}
\setnotesheader{Graduate Discrete Math}
...
\begin{proof}
    ...
    Now choose $\sigma$ uniformly at random and let $\mathcal{X}=|\mathcal{H}_\sigma\cap F|$. Then $\mathcal{X}$ is a random variable and $\mathcal{X}\leq 1.$\\
...
\begin{lem}
    Adjacent transposition generates $S_n.$
\end{lem}
\begin{proof}
...
\begin{proof}
    ...
    If $p^{\epsilon'}n\to 0,$ there is some argument for densest subgraph $J.$
\end{proof}        
\begin{proof}

...
\author{Wilson Pan}
\date{\today}
\setnotesheader{Convex Optimization}
...
\section{Jan 13: Introduction to Optimization}

\subsection{Basic Definitions}
\begin{defn}
    A \textbf{optimization problem} is defined as
    \[
...
\newpage
\section{Jan 15: Convex Sets and Functions}
...
\subsection{Local vs.\ Global Optima}
\begin{thm}
...
\newpage
\section{Jan 20: Convex Combinations, Hulls, and Cones}

\subsection{Types of Combinations}
\begin{defn}
...
\subsection{Norms and Convex Sets}
\begin{defn}
...
\subsection{Optimality and the Normal Cone}
\begin{thm}
...
\newpage
\section{Jan 22: Optimality Conditions and Characterizations of Convexity}

\subsection{Optimality and Operations on Convex Sets}
\begin{thm}
...
\newpage
\section{Jan 27: Matrix Theory, Lipschitz Continuity, and Smoothness}

\subsection{Gradient Monotonicity and SVD}
\begin{defn}
...
\end{thm}
\newpage
\section{Feb 3: Gradient Descent and Convergence}

\subsection{Descent Directions and Step Sizes}
\begin{defn}
...
\end{defn}
\begin{thm}
    Assume $f$ is $\beta$-smooth, $\alpha$-strongly convex, $\eta=\frac{1}{\beta}$ then \[
...
\end{proof}
\newpage
\section{Feb 5: Subgradients and the Subgradient Method}

\subsection{Review and Subgradient Calculus}
\begin{rec}
    From previous class
    \begin{enumerate}
...
\begin{defn}
    \textbf{Subgradient Method:} $x_{t+1}\leftarrow x_t-\eta g_x$ for some $g_x\in \partial f(x)$
    \begin{enumerate}
...
\end{eg}
\newpage
\section{Feb 10: Subgradient Method Analysis and Projections}

\subsection{Subgradient Method Rates}
\begin{defn}
...
\begin{thm}
    ...
    \]
\end{thm}
\begin{proof}[Verification of Theorem 8.2]
    Set $\eta_t = \frac{R}{G\sqrt{T}}$ for all $t$. With $\lVert x_0 - x^* \rVert \leq R$:
    \begin{align*}
        \sum_{t=1}^T \eta_t &= T \cdot \frac{R}{G\sqrt{T}} = \frac{R\sqrt{T}}{G}, \\
        \sum_{t=1}^T \eta_t^2 &= T \cdot \frac{R^2}{G^2 T} = \frac{R^2}{G^2}.
    \end{align*}
    Substituting into the bound (using $\lVert x_0 - x^* \rVert^2 \leq R^2$):
    \begin{align*}
        f \left( x_T^{\mathrm{(best)}} \right) - f(x^*)
        &\leq \frac{R^2 + G^2 \cdot \frac{R^2}{G^2}}{2 \cdot \frac{R\sqrt{T}}{G}}
        = \frac{2R^2}{2R\sqrt{T}/G}
        = \frac{2R^2 \cdot G}{2R\sqrt{T}}
        = \frac{GR}{\sqrt{T}}.
    \end{align*}
    Hence Theorem 8.2 holds with the $O(1/\sqrt{T})$ rate.
\end{proof}
\begin{proof}
    \begin{align*}
    ...
    .\] 
    So \[
    2\sum_{t=0}^{T-1}\eta_t \left( f(x_T^{\mathrm{(best)}})-f(x^*) \right)\leq2\sum_{t=0}^{T-1}\eta_t \left( f(x_t)-f(x^*) \right)\leq ||x_0-x^*||^2+G^2 \sum_{t=0}^{T-1}\eta_t^2
    .\] 
\end{proof}

\subsection{Polyak Stepsize and Feasibility}
\begin{thm}
...
\begin{proof}
    ...
    \begin{lem}
        $u$ is the projection of $x$ onto $C$ iff $\langle x-u,y-u\rangle \leq 0 \text{ }\forall y\in C$
    \end{lem} 
...
\end{proof}
\newpage
\section{Feb 12: Projected Subgradient Method}

\subsection{Projections and Contractions}
\begin{defn}
...
\end{thm}
\begin{proof}
    We have $z=P_c(x)$ iff $\forall y\in C$, $\langle x-z,y-z\rangle \leq 0$.
    Then for \[
...
\begin{proof}
    We have 
    \begin{enumerate}
        \item $\forall z_2\in C, \inner{x_1-z_1}{z_2-z_1} \leq 0 $
        \item $\forall z_1\in C, \inner{x_2-z_2}{z_1-z_2} \leq 0 $
        \item Adding these two inequalities we have \[
        \inner{x_1-z_1}{z_2-z_1}+\inner{x_2-z_2}{z_1-z_2}=\inner{x_1-x_2+z_2-z_1}{z_2-z_1} \leq 0
        .\]
    \end{enumerate}
    So \[
        \inner{x_1-x_2}{z_2-z_1}+||z_2-z_1||_2^2\leq 0\iff ||z_2-z_1||_2^2\leq \inner{x_1-x_2}{z_2-z_1}\leq ||x_1-x_2||||z_2-z_1||
        .\] 
        So we have our result \[
        \norm{z_1-z_2}\leq ||x_1-x_2||
        .\] 
\end{proof}

\subsection{Convergence and Examples}
\begin{thm}
    Rates for PGD are identical to GD and similarly PSGM is identical to SGM. 
\end{thm}
\begin{proof}
    \[
    ||x_{t+1}-x^*||^2=||P_C(x_t-\eta_t g_t)-x^*||^2=||P_C(x_t-\eta_t g_t)-P_C(x^*)||\leq ||x_t-\eta_tg_t-x^*||^2
    .\] 
\end{proof}
\begin{eg}
    Consider \[
    \min_{x\in \R^d} \frac{1}{2}||Ax-b||_2^2 \text{ where } n\geq d \text{ and } A \text{ full rank }
    .\] 
    When we solve this by gradient descent then the rate would be \[
    \left( 1-\frac{\lambda_{\text{min}}}{\lambda_{\text{max}}} \right)^k
    .\] 
\end{eg}
\begin{proof}
    We have $\nabla f(x)=A^T(Ax-b)$ and $\nabla^2 f(x)=A^TA$. So $f$ is quadratic with Hessian $A^TA$. The eigenvalues of $A^TA$ satisfy $\lambda_{\min}=\lambda_{\min}(A^TA)$ and $\lambda_{\max}=\lambda_{\max}(A^TA)$. Hence $f$ is $\alpha$-strongly convex with $\alpha=\lambda_{\min}$ and $\beta$-smooth with $\beta=\lambda_{\max}$. Taking $\gamma=\beta/\alpha=\lambda_{\max}/\lambda_{\min}$ and $\eta=1/\beta$, the strongly convex convergence theorem gives \[
    ||x_k-x^*||^2\leq \left(1-\frac{1}{\gamma}\right)^k ||x_0-x^*||^2
    =\left(1-\frac{\lambda_{\min}}{\lambda_{\max}}\right)^k ||x_0-x^*||^2.
    \]
\end{proof}
\begin{eg}
...
\end{eg}
\begin{eg}
    Example of Fast Projection:
    \[
    C= \left\{Ay+b | y\in \R^\ell \right\}\subseteq \R^n \text{ and } P_C(x)=b+A(A^TA)^{-1}A^T(x-b)
    .\] 
\end{eg}
\begin{proof}
    \[
    y^*=\arg\min_{y\in \R^\ell} \frac{1}{2}||x-(Ay+b)||_2^2\implies A^T(Ay^*+b)-x=0 \implies A^TAy^*=A^T(x-b)\implies y^*=(A^TA)^{-1}A^T(x-b)
    .\] 
    So $P_C(x)=Ay^*+b$
\end{proof}
\begin{eg}
    \[
        C= \left\{y: Ay=b\right\}\subseteq R^d \text{ and } P_c(x)=x+A^T(AA^T)^{-1}(b-Ax)
    .\] 
\end{eg}
\newpage
\section{Feb 14: Optimality Conditions and Proximal Gradient Descent}
\begin{thm}
    Consider $\min_{x \in \R^d} f(x)$, $f$ not assumed convex. Then $x^*$ is optimal on $\dom(f)$ iff $0 \in \partial f(x^*)$.
\end{thm}
\begin{proof}
($\impliedby$) We have \[
f(y)\geq f(x^*)+\inner{g_{x^*}}{y-x^*}\tag{$\forall y\in \dom(f)$ and $g_x^*\in \partial f(x^*)$ }
.\] 
Since $0\in \partial f(x^*)$ we have $f(y)\geq f(x^*)$ for all $y\in \dom(f)$. So $x^*$ is optimal.\\
($\implies$) \[
f(y)\geq f(x^*)+\inner{0}{y-x^*}\implies 0\in \partial f(x^*)
.\] 
\end{proof}
\begin{thm}
    $f$ is convex and differentiable with $C$ convex then \[
    x^*\in \arg\min_{x\in C}f(x) \iff -\nabla f(x^*)\in N_C(x^*)
    .\] 
\end{thm}
\begin{thm}
    If $f$ is convex and $C$ convex then \[
    x^* \text{ is optimal for $f$ in $C$} \iff 0\in \partial f(x^*)+N_C(x^*)
    .\] 
\end{thm}
\begin{proof}
    We can rewrite the objective as \[
    x^*\in \arg\min_{x\in C}f(x)=\arg\min_{x\in \R^d}f(x)+I_C(x) \iff 0\in \partial f(x^*)+N_C(x^*)
    .\] 
\end{proof}
\begin{eg}
    Soft Thresholding: Consider the optimization problem with an $L_2$ data fidelity term and an $L_1$ regularization term (Lasso):
    \[
        x^* = \underset{z \in \mathbb{R}^d}{\arg\min} \left( \frac{1}{2}\|z - y\|_2^2 + \lambda\|z\|_1 \right).
    \]
    ...
    \begin{enumerate}
        \item \textbf{Case 1 ($x_j^* > 0$):} The subgradient is $\{1\}$.
        \[
        ...
        \item \textbf{Case 2 ($x_j^* < 0$):} The subgradient is $\{-1\}$.
        \[
            x_j^* - y_j - \lambda = 0 \implies x_j^* = y_j + \lambda.
        \]
        For this to be consistent with $x_j^* < 0$, we must have $y_j + \lambda < 0$, or $y_j < -\lambda$.
    
        \item \textbf{Case 3 ($x_j^* = 0$):} The subgradient is the interval $[-1, 1]$.
        The condition becomes $0 \in -y_j + \lambda [-1, 1]$, which means $y_j \in [-\lambda, \lambda]$.
        This holds whenever $|y_j| \le \lambda$.
    \end{enumerate}
    
    Combining these cases yields the closed-form \textbf{Soft Thresholding Operator}, denoted as $\mathcal{S}_\lambda(y)$:
    \[
        x_j^* = \mathcal{S}_\lambda(y_j) = 
        \begin{cases} 
            y_j - \lambda & \text{if } y_j > \lambda \\
            y_j + \lambda & \text{if } y_j < -\lambda \\
            0 & \text{if } |y_j| \le \lambda
        \end{cases}
    \]
...
\end{eg}
\begin{thm}
    Proximal Gradient Descent: Our objective is \[
    \min_{x\in \R^d}\underbrace{g(x)}_{\substack{convex \\ diff}} +\underbrace{h(x)}_{\substack{convex \\ non-diff\\ simple}}
    .\] 
    Our update rules are 
    \begin{align*}
        y^{t+1}&\leftarrow x^t-\eta_t \nabla g(x^t)\\
        x^{t+1}&\leftarrow prox_{\eta_t,h}(y^{t+1}):=\arg\min_{z\in \R^d} \frac{1}{2\eta_t}||y^t-z||_2^2+h(z)
    \end{align*}
    When $g(x)=||Ax-b||_2^2$, $h(x)=\lambda||x||_1$ then this is Proximal GD called ISTA. 
\end{thm}
\begin{rmk}
    The intuition is that \[
    x_{t+1}\leftarrow \arg\min_z g(x_t)+\nabla g(x_t)^T(z-x_t)+\frac{1}{2\eta_t}||z-z_t||_2^2+h(z)=\arg\min_{z}\frac{1}{2\eta_t}||z-(x_t-\eta_t\nabla g(x_t))||_2^2+h(z)
    .\] 
\end{rmk}
\begin{thm}
    For $p(x)=Prox_{\eta, h}(x)=\arg\min_{z}\frac{1}{2\eta}||x-z||_2^2+h(z)$ we have 
    \begin{enumerate}
        \item $||p(x)-p(y)||_2^2\leq \inner{x-y}{p(x)-p(y)}$
        \item $||p(x)-p(y)||\leq ||x-y||$
    \end{enumerate}
...
\begin{proof}
    ...
    \inner{g_x-g_y}{x-y}\geq 0 \text{ for all } x,y \text{ and } g_x\in\partial f(x),g_y\in \partial f(y)
    .\] 
    So \[
    \inner{ \frac{1}{\eta}(y-p(y))-\frac{1}{\eta}(x-p(x))}{p(y)-p(x)}\geq 0 \iff \inner{y-x}{p(y)-p(x)}\geq ||p(y)-p(x)||_2^2 
    .\] 
\end{proof}
\begin{thm}
    Define $G_\eta(x)=\frac{x-prox_{\eta,h}(x-\eta\nabla g(x))}{\eta}$ then the following are true:
    \begin{enumerate}
        \item $G_\eta(x)=0 \iff x=prox_{\eta,h}(x-\eta\nabla g(x))$ 
        \item $G_\eta(x^*)=0 \iff 0\in \nabla g(x^*)+\partial h(x^*)\iff x^* \text{ is optimal} \iff x^* \text{ is a fixed point of prox}$
    \end{enumerate}
\end{thm}
\begin{proof}
    By definition $\forall \tilde{x}$ then \[
        \tilde{x}-\eta G_\eta(\tilde{x})=prox_{\eta,h}(\underbrace{\tilde{x}-\eta\nabla g(\tilde{x})}_{x})
    .\] 
    \begin{align*}
        u:=prox_{\eta,h}(x)&\iff \frac{1}{\eta}(x-u)\in \partial h(u)\\
        &\iff \tilde{x}-\eta\nabla g(\tilde{x})-\tilde{x}-\eta G_\eta(\tilde{x})\in \eta \partial h(\tilde{x}-\eta G_\eta(\tilde{x}))\\
        &\iff G_\eta(\tilde{x})\in \nabla g(\tilde{x})+\partial h(\tilde{x}-\eta G_\eta(\tilde{x})) \text{ } \forall \tilde{x}
    \end{align*}
    Thus we imply $1$ by choosing $\tilde{x}=x^*$
\end{proof}
\newpage
\section{Feb 19: Stochastic Gradient Descent}
\begin{defn}
    Stochastic Gradient Descent: We have access to $g(x_t,\zeta_t)$ unbiased for $\nabla f(x_t)$ or an element of $\partial f(x_t)$. 
    \[
...
\end{defn}
\begin{eg}
    $R_n(\theta)=\frac{1}{n}\sum_{i=1}^n \ell(f_\theta(x_i,y_i))$, if $n$ is large, computing $\nabla R_n$ is expensive. \[
        g_i=\nabla \ell (f_\theta(x_i),y_i) \text{ is unbiased for } \nabla R_n(\theta) \text{ if } i \sim \text{Unif} \{1,...,n\}
    .\]  
\end{eg}
\begin{thm}
    Assume $f$ is convex, $||x_0-x^*||=R$ and $\E[||g(x_t,\zeta_t)||_2^2]\leq G^2$ then choosing $\eta=\frac{R}{G\sqrt{k}}$ yields \[
    \E\left [f\left (\frac{1}{k}\sum_{i=1}^k x_i\right )-f(x^*)\right ]\leq \frac{RG}{\sqrt{k}}
    .\] 
\end{thm}
\begin{proof}
    Define $g_t=\E[\tilde{g}_t]$ then we have \[
    ||x_{t+1}-x^*||^2=||x_t-\eta_t \tilde{g}_t-x^* ||^2=||x_t-x^*||^2-2\eta_t \tilde{g}_t^T(x_t-x^*)+\eta_t^2 g_t^2
    .\] 
    So 
    \begin{align*}
        \E[||x_{t+1}-x^*||_2^2 \mid  x_t]&\leq ||x_t-x^*||^2-2\eta_tg_t^T(x_t-x^*)+\eta_t^2G^2\\
        &\leq ||x_t-x^*||^2-2\eta_t(f(x_t)-f(x^*))+\eta_t^2G^2\\
        \E[||x_{t+1}-x^*||_2^2]&\leq \E[||x_t-x^*||^2]-2\eta_t\E[f(x_t)-f(x^*)]+\eta_t^2G^2\\
    \end{align*}
    So we can conclude \[
    0\leq \E[||X_{t+1}-x^*||_2^2]\leq R^2-2\sum_{i=1}^k \E[f(x_i)-f(x^*)]+\sum_{t=1}^k\eta_t^2G^2
    .\] 
    Rearranging we have \[
    2\sum_{i=1}^k \eta_t \E[f(x_i)-f(x^*)]\leq R^2+\sum_{t=1}^k\eta_t^2G^2
    .\] 
    For $\eta_t=\eta$ we have \[
    \frac{1}{k}\sum_{i=1}^k \E[f(x_i)-f(x^*)]\leq \frac{1}{k}\left (\frac{R^2}{2\eta}+\frac{\eta kG^2}{2}\right )
    .\] 
    So $\eta^2=\frac{R^2}{kG^2}\implies \eta=\frac{RG}{\sqrt{k}}$. \\
    Proof concludes by $f \left( \frac{1}{k}\sum_{i=1}^k x_i\right )\leq \frac{1}{k}\sum_{i=1}^k f(x_i)$.
\end{proof}
\begin{thm}
    Assume $f$ is $\alpha$-strongly convex. For any $\eta<\frac{1}{\alpha}$ we have \[
    \E \left( ||x_k-x^*||_2^2 \right)\leq (1-\eta \alpha)^k ||x_0-x^*||_2^2+\frac{G\eta}{\alpha}
    .\] 
\end{thm}
\newpage
\section{Feb 24:}
\begin{rec}
    $\min_{x\in \R^d} g(x)+h(x)$ where $g$ is $\beta$-smooth and convex and $h(x)$ is non-smooth, convex, and simple. \\
    \[
        Prox_{\eta,h}(x)=\arg\min_{z\in \R^d} \frac{1}{2\eta}||x-z||_2^2+h(z)
    .\]
    We define the update rules as 
    \[
    y_{t+1}\leftarrow x_t-\eta_t \nabla g(x_t) \text{ and } x_{t+1}\leftarrow Prox_{\eta_t,h}(y_{t+1})
    .\] 
\end{rec}
\begin{lem}
    Descent Lemma For $\eta\leq \frac{1}{\beta}$ and for any $z$, 
    \[
    f(x-\eta G_\eta(x))\leq f(z)+G_\eta(x)^T(x-z)-\frac{\eta}{2}||G_\eta(x)||_2^2-\frac{\alpha}{2}||x-z||_2^2
    .\] 
\end{lem}
\begin{rmk}
    When $z=x$ we have 
    \[
    f(x_{t+1})\leq f(x_t)- \frac{\eta}{2}||G_\eta(x_t)||_2^2
    .\] 
\end{rmk}
\begin{thm}
    For $\eta=\frac{1}{\beta}$ we have \[
    f(x_k)-f(x^*)\leq \frac{\beta ||x_0-x^*||_2^2}{2k}
    .\] 
\end{thm}
\begin{proof}
    From the Descent Lemma, if we let $z=x^*$ we have 
    \[
    f(x_{t+1})\leq f(x^*)+G_\eta(x_t)^T(x_t-x^*)-\frac{\eta}{2}||G_\eta(x_t)||_2^2
    .\] 
    \begin{align*}
        ||x_{t+1}-x^*||_2^2&=||x_t-x^*||-2\eta G_\eta (x_t)^T(x_t-x^*)+\eta^2 ||G_\eta (x_t)||_2^2\\
        &\leq ||x_t-x^*||_2^2+2\eta(f(x^*)-f(x_{t+1}))
    \end{align*}
    Rearranging we have 
    \[
    f(x_k)-f(x^*)\leq f(x_{t+1})-f(x^*)\leq \frac{1}{2\eta} \left( ||x_t-x^*||_2^2-||x_{t+1}-x^*||_2^2 \right)
    .\] 
    Telescoping for $t=0,...,k-1$ we have 
    \[
    k(f(x_k)-f(x^*))\leq \frac{1}{2\eta} \left( ||x_0-x^*||_2^2-||x_{k+1}-x^*||_2^2 \right)
    .\] 
    So \[
    f(x_k)-f(x^*)\leq \frac{\beta ||x_0-x^*||_2^2}{2k}
    .\] 
\end{proof}
\begin{defn}
    Let $f:C\to \R$ be non-convex then $g$ is called the convex envelope of $f$ if it is the largest convex function that lies below $f$. Formally $g(x)\leq f(x)$ for all $x\in C$. 
\end{defn}
\begin{eg}
    Matrix Completion (Netflix Problem): Given a $n\times m$ matrix $M^*$ with $O(n+m)$ non-zero entries. Let $\mathcal{I}$ be the observed indices then the set of observed values is $\{y_{i,j}| (i,j)\in \mathcal{I}\}$. As of right now we can't recover the matrix so if we instead assume $M^*$ is a low rank matrix then we can write 
    \[
    \hat{M}^*=\arg\min_{X}\sum_{(i,j)\in \mathcal{I}}(M_{i,j}-y_{i,j})^2+\lambda ||M||_*
    .\]
    Then
    \[
    Prox_{\eta, *}(L)=\arg\min_M \frac{1}{2}||M-L||_2^2+\eta||M||_*=U\Sigma_\eta V^T \tag{where$L=U\Sigma V^T$}
    .\]  
\end{eg}
\begin{thm}
    Let $s(x)=rank(X)$, $C= \{X: ||X||_2\leq 1\}$ with $t(X)\leq s(X)$ for all $X\in C$ then 
    \[
        t(X)=||X||_*=\sum_{i=1}^r \sigma_i=tr(\sqrt{X^TX})\leq rank(x)
    .\] 
\end{thm}
\newpage
\section{March 10: Duality}
\begin{defn}
    At a high level, it gives you, every dual feasible point gives a provable lower bound of the original primal problem, even if the original (primal) problem is non-convex. (Weak Duality) 
\end{defn}
\begin{defn}
    A LP is simply an optimization problem \[
    \min_{x\in \R^d} c^T x \text{ subject to } Ax=b, Gx\leq h
    .\] 
\end{defn}

\subsection{Primal and Dual LP: An Example}
For any $a,b,c \geq 0$ with $a+b=p$ and $a+c=q$, we obtain the lower bound
\[
    \min_{x,y} \{ px + qy : x+y \geq 2,\; x,y \geq 0 \} \geq 2a
\]
\textbf{What's the best we can do?} Maximize our lower bound over all feasible $a,b,c$:
\begin{center}
    \textbf{Primal LP} \hfill \textbf{Dual LP}
\end{center}
\begin{align*}
    \min_{x,y} \quad & px + qy & \qquad \qquad & \max_{a,b,c} \quad & 2a \\
    \text{subject to } \quad & x + y \geq 2 & & \text{subject to } \quad & a + b = p \\
    & x, y \geq 0 & & & a + c = q \\
    & & & & a, b, c \geq 0
\end{align*}
\begin{rmk}
    ...
    \begin{itemize}
        \item The number of dual variables equals the number of primal constraints (3 in this example).
        \item Weak duality: $\min_{x,y} px + qy \geq \max_{a,b,c} 2a$.
    \end{itemize}
\end{rmk}
\begin{thm}
    Given $c\in \R^n,A\in \R^{m\times n},b\in \R^m,G\in \R^{r\times n},h\in \R^r$ then for the primal LP 
    \[
    \min_{x\in \R^n} c^T x \text{ subject to } Ax=b, Gx\leq h
    .\] 
    Then the dual LP is 
    \[
    \max_{u\in \R^m,v\in \R^r} -b^T u-h^Tv \text{ subject to } -A^T u - G^T v = c, v\geq 0
    .\] 
\end{thm}
\begin{proof}
    For any $u\in \R^m,v\in \R^r$, $v\geq 0$ and $x$ primal feasible, we have 
    \begin{align*}
        u^T(Ax-b)+v^T(Gx-h)&\leq 0 \\
        u^TAx-u^Tb+v^TGx-v^Th&\leq 0 \\
        (u^T A + v^T G)x &\leq u^T b + v^T h \\
        (A^T u + G^T v)^T x &\leq b^T u + h^T v \\
        (-A^T u - G^T v)^T x &\geq -b^T u - h^T v
    \end{align*}
    If $c=-A^T u - G^T v$ and $x$ primal feasible
    \[
    c^T x\geq -b^T u - h^T v
    .\]  
    and we have a lower bound on the primal feasible optimal value $c^T x^*$.
\end{proof}
\begin{rmk}
    The dual is also a linear program. It is a maximization program 
\end{rmk}
\begin{rmk}
    Each constraint in the primal, yields a variable in the dual and each variable in the dual will yield a constraint in the primal. 
\end{rmk}
\begin{rmk}
    By construction if we denote the primal optimal value as $p^*$ and the dual optimal value as $d^*$ then we have $p^*\geq d^*$. This is known as weak duality. \\
    Strong duality: $p^*=d^*$
\end{rmk}
\begin{rmk}
    Another perspective on LP duality, given $c\in \R^n, A\in \R^{m\times n},b\in \R^m,G\in \R^{r\times n},h\in \R^r$ then for the primal LP 
    \[
    \min_{x\in \R^n} c^T x \text{ subject to } Ax=b, Gx\leq h 
    .\] 
    ...
    .\] 
    So if $C$ denotes primal feasible set: $C= \{x | x\in \R^n, Ax=b, Gx\leq h\}$, $f^*$ is the primal optimal value: $f^*=\min_{x\in C}c^T x$ 
    \[
    f^*=\min_{x\in C}c^Tx\geq \min_{x\in C}L(x,u,v)\geq \min_{x\in \R^n}L(x,u,v):=g(u,v)
    .\]
    \[
    g(u,v)=\min_{x\in \R^n}(c+A^Tu+G^Tv)^Tx-b^Tu-h^Tv= \begin{cases}
        -\infty & \text{if } c+A^Tu+G^Tv\neq 0 \\
        -b^Tu-h^Tv & \text{if } c+A^Tu+G^Tv= 0
    \end{cases}
    .\]  
    We can maximize $g(u,v)$ over $u$ and $v\geq 0$ to get the tightest bound, and this gives exactly the dual LP as before 
\end{rmk}

...
\begin{thm}
    Consider $\min_x\in \R^df(x), f$ not assumed convex. $x^*$ is optimal on $\dom(f)\iff 0\in \partial f(x^*)$
\end{thm}
\begin{proof}
    $(\impliedby)$ $f(y)\geq f(x^*)+\inner{g_{x^*},y-x^*}$ for all $g_{x^*}\in \partial f(x^*)$ and $y\in \dom(f)$. 
\end{proof}
\end{document}

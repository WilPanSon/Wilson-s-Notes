...
\begin{proof}
    \begin{align*}
        ...
        &\leq ||x_t-x^*||^2+\eta_t^2G^2+2\eta_t (f(x^*)-f(x_t)) \tag{*}
    \end{align*}
...
\end{thm}
\begin{thm}
    Given $C_1,...,C_k$ convex sets find $x^*\in \bigcap_{i=1}^k C_i$. Define $f_i(x)=\min_{y\in C_i}||x-y||^2=\mathrm{dist}(x,C_i)$ and $f(x)=\max_{i=1,...,k}f_i(x).$
\end{thm}
\end{document}

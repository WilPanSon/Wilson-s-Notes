...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[scale=1.1, >=Stealth]
        % Level sets of convex f(x) = (x1)^2 + (x2)^2 (ellipses scaled)
        \begin{scope}
            \foreach \r in {0.3, 0.6, 1.0, 1.5, 2.2} {
                \draw[gray!50, thin] (0,0) ellipse ({\r*1.2} and {\r*0.8});
            }
        \end{scope}
        % x^* at minimum
        \fill (0,0) circle (2pt);
        ...
        \node[above right] at (1.8, 1.2) {$x_0$};
        % Subgradient path (zigzag - subgradients aren't descent directions)
        \draw[thick, blue!70!black, ->] (1.8, 1.2) -- (1.4, 0.5);
        \draw[thick, blue!70!black, ->] (1.4, 0.5) -- (1.9, 0.1);
        \draw[thick, blue!70!black, ->] (1.9, 0.1) -- (1.2, -0.3);
        \draw[thick, blue!70!black, ->] (1.2, -0.3) -- (0.5, 0.2);
        \draw[thick, blue!70!black, ->] (0.5, 0.2) -- (0.2, -0.1);
        % x_T (last iterate)
        \fill[blue!70!black] (0.2, -0.1) circle (2pt);
        \node[below left] at (0.2, -0.1) {$x_T$};
        % x_T^(best) - best along path (closest to center = lowest f)
        \fill[red!80!black] (0.5, 0.2) circle (2.5pt);
        \node[above right, red!80!black] at (0.5, 0.2) {$x_T^{\mathrm{(best)}}$};
        % ||x_0 - x^*|| brace
        \draw[thick, decorate, decoration={brace, amplitude=5pt, mirror}] (0,0) -- (1.8, 1.2)
            node[midway, below=8pt, sloped] {$\lVert x_0 - x^* \rVert$};
        % Optimality gap: f(x_T^best) - f(x^*) bounded; level sets show f-values
        \draw[dashed, red!70] (0.5, 0.2) -- (0, 0);
        \node[red!80!black, font=\footnotesize, above right] at (0.25, 0.1) {$f(x_T^{\mathrm{(best)}})-f(x^*)$};
        % Formula callout
        \node[draw, rounded corners, inner sep=6pt, font=\small, align=center] at (2.8, -1.2) {
            bound $\propto \dfrac{\lVert x_0{-}x^*\rVert^2 + G^2\sum\eta_t^2}{\sum\eta_t}$
        };
    \end{tikzpicture}
    \caption{Subgradient method: the path may zigzag (subgradients need not be descent directions). The \textbf{best iterate} $x_T^{\mathrm{(best)}}$ minimizes $f$ over $\{x_0,\ldots,x_T\}$. The bound shrinks when $\sum\eta_t$ is large (more ``progress'') and when $\lVert x_0{-}x^*\rVert$ and $\sum\eta_t^2$ are small.}
\end{figure}
\begin{proof}
    \begin{align*}
...
\begin{proof}
    ...
    So $P_C(x)=Ay^*+b$
\end{proof}
\begin{TheoremBox}[Another Example of Fast Projection]
    \[
    C= \left\{y: Ay=b\right\}\subseteq R^d \implies n\leq d
    .\] 
\end{TheoremBox}
\end{document}

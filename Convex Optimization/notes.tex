...
\newcommand{\envbox}[1]{%
    \begin{tcolorbox}[
        ...
        boxrule=0pt,
        frame hidden,
        borderline west={4pt}{0pt}{#1!80!black},
        left=6pt,
        right=6pt,
        top=4pt,
        bottom=4pt,
...
\AtBeginEnvironment{lem}{\envbox{OliveGreen}}
...
\begin{proof}
    ...
    .\] 
    So \[
    \inner{ \frac{1}{\eta}(y-p(y))-\frac{1}{\eta}(x-p(x))}{p(y)-p(x)}\geq 0 \iff \inner{y-x}{p(y)-p(x)} \geq \inner{y-x}{p(y)-p(x)}\geq ||p(y)-p(x)||_2^2 
    .\] 
\end{proof}
\begin{thm}

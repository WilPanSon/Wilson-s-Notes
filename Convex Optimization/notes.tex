...
\AtEndEnvironment{rec}{\end{tcolorbox}}x`'
...
\begin{rmk}
    ...
    x_{t+1}\leftarrow \arg\min_z g(x_t)+\nabla g(x_t)^T(z-x_t)+\frac{1}{2\eta_t}||z-z_t||_2^2+h(z)=\frac{\arg\min_{z}1}{2\eta_t}||z-(x_t-\eta_t\nabla g(x_t))||_2^2+h(z)
    .\] 
\end{rmk}
\begin{thm}
    For $p(x)=Prox_{\eta, h}(x)=\frac{\arg\min_{z}1}{2\eta}||x-z||_2^2+h(z)$ we have 
    \begin{enumerate}

...
\end{defn}
\begin{eg}
    Matrix Completion (Netflix Problem): Given a $n\times m$ matrix $M^*$ with $O(n+m)$ non-zero entries. Let $\mathcal{I}= \{y_{i,j}| (i,j)\in \mathcal{I}\}$ be the observed indices. As of right now we can't recover the matrix so if we instead assume $M^*$ is a low rank matrix then we can write 
    \[

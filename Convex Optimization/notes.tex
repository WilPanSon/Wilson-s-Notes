...
\begin{thm}
    ...
    \min_{x\in \R^d}\underbrace{g(x)}_{\substack{convex \\ diff}} +\underbrace{h(x)}_{\substack{convex \\ non-diff}}
    .\] 
    Our update rules are 
    \begin{align*}
        y^{t+1}&\leftarrow x^t-\eta_t \nabla g(x^t)\\
        x^{t+1}&\leftarrow prox_{\eta_t,h}(y^{t+1}):=\arg\min_{z\in \R^d} \frac{1}{2}||y-z||_2^2+h(z)
    \end{align*}
    When $g(x)=||Ax-b||_2^2$, $h(x)=\lambda||x||_1$ then this is Proximal GD 
\end{thm}
\end{document}

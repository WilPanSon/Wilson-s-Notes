...
\begin{thm}
    ...
    \min_{x\in \R^d}\underbrace{g(x)}_{convex \& diff} +\underbrace{h(x)}_{convex \& non-diff}
    .\] 
    Our update rules are 
    \begin{align*}
        y^{t+1}&\rightarrow x^t-\eta_t \nabla g(x^t)\\
        x^{t+1}&\rightarrow prox_{\eta_t,h}(y^{t+1}):=\arg\min_{z\in \R^d} \frac{1}{2}||y-z||_2^2+h(z)
    \end{align*}
    \[
    y^{t+1}\rightarrow x^t-\eta_t \nabla g(x^t)
    .\] 
